...
function create_user()
	{
		...
		$data['user_status'] = $this->input->post('status');
	...
	function update_user_gate($id = "")
	{
	    $data['gate_no'] = $this->input->post('gate_no');
	   
	    $user_name = $this->db->where('id', $id)->get('users')->row()->name;
	    ...
	    $this->db->update('users', $data);
			...
			$log = array(
				'name' => 'gate_assign',
				'description' => ''.$user_name.' has assign gate no: '.$data['gate_no'].'  Updated successfully.',
			 );
			 app_log($log);  
