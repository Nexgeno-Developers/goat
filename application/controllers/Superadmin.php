...
// ============== 2024 ==========================
  ...
  public function excel_import_user(){
    ...
    $csvPath = FCPATH . 'uploads/gawal.csv';
    ...
    while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
        ...
        $userData = [
            ...
            'applicant_name' => $data[4],
            'valid_from'        => $data[1], 
            'valid_to'        => $data[2], 
            'application_date'  => $data[5],
            'application_status'    =>  $data[6],
            'license_type'   =>  $data[8],
            'fees'     =>  $data[7],
        ];

        // Check if email already exists
        $this->db->where('license_no', $userData['license_no']);
        $query = $this->db->get('app_gwala');

        if ($query->num_rows() == 0) {
            $this->db->insert('app_gwala', $userData);
            $inserted++;
        } else {
            echo "⚠️ Duplicate License No: " . $userData['license_no'] . "<br>";

            $this->db->where('license_no', $userData['license_no']);
            $this->db->update('app_gwala', $userData);
            echo "🔁 Updated Existing Gawal: " . $userData['license_no'] . "<br>";
    ...
    echo "✅ Total records read From Gawal Excel: $rowCount<br>";
    echo "✅ Inserted Excel Data in Gawal: $inserted<br>";
    echo "✅ Updated Records in Gawal: $duplicates<br>";
